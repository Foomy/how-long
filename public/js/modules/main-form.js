...
const initRemoveRowButtons = (document) => {
    const deleteButtons = document.querySelectorAll('button[data-action="remove-row"]');

    deleteButtons.forEach((delBtn) => {
        delBtn.removeEventListener('click', handleRemoveRow);
        delBtn.addEventListener('click', handleRemoveRow)
    });
};
...
const cloneElement = (element, type, rowNumber) => {
    ...
    cloned.childNodes.forEach((item) => {
        ...
        if ('BUTTON' === item.nodeName) {
            item.setAttribute('data-js', 'remove-row');
            item.setAttribute('data-rownum', rowNumber);
        }
    });
